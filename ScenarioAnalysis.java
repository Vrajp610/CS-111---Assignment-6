    ...
    public void computeCO2EmissionsAndCost () {
        for (Vehicle t : getVehicles()){
            if(t.getFuel().getType() == 2){
                double milesPerKilowatt = t.getFuel().getUsage();
                double kilowattPerHourCharge = t.getFuel().getKWhPerCharge();
                double electricPrice = getElectricityPrice() / 100;
                
                int totalLeaseYears = t.getLease().getNumberOfMonths() / 12;
                int milesTotal = t.getLease().getMileageAllowance() * totalLeaseYears;
                
                double totalCostOfLease = t.getLease().getDueAtSigning() + (t.getLease().getMonthlyCost()* t.getLease().getNumberOfMonths());
                double totalCarbonEmission = ((milesTotal/milesPerKilowatt) * kilowattPerHourCharge) * (Fuel.CO2EMITTED_GENERATEmWh/1000) * 0.45;
                
                t.setCO2Emission(totalCarbonEmission);
                t.setFuelCost((milesTotal/milesPerKilowatt) * (electricPrice * kilowattPerHourCharge));
                t.setTotalCost(totalCostOfLease + t.getFuelCost()+ t.getOtherCost());
                ...
                double totalCostOfLease = t.getLease().getDueAtSigning() + (t.getLease().getMonthlyCost()* t.getLease().getNumberOfMonths());
                double totalCostOfFuel = milesTotal/ t.getFuel().getUsage() * this.getGasPrice();
                double totalCarbonEmission = milesTotal/ t.getFuel().getUsage() * Fuel.CO2EMITTED_GASCOMBUSTION;
                
                t.setCO2Emission(totalCarbonEmission);
                t.setFuelCost(totalCostOfFuel);
                t.setTotalCost(totalCostOfFuel+ totalCostOfLease + t.getOtherCost());
            }
        }
